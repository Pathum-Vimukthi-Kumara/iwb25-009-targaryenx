...
const AdminEvents = () => {
  ...
  const fetchEvents = async () => {
    ...
    try {
      ...
      if (eventsData) {
        ...
        setError(
          "CORS issue detected: Unable to connect to the API due to cross-origin restrictions. Make sure the backend server is running and has proper CORS headers. Showing sample data instead."
        );
      }
    } catch (error) {
      console.error("Error fetching events:", error);
      setError(
        "API connectivity issue: Could not connect to the events API. This could be due to CORS restrictions or the backend server not running. Using sample data for development."
      );
  ...
  const viewEventDetails = async (event) => {
    ...
    // fetch organization name for the selected event
    if (event && event.organization_id) {
      setOrgLoading(true);
      setOrgError(null);
      try {
        const userData = await fetchWithFallback(
          `pub/users/${event.organization_id}`
        );
        if (
          userData &&
          (userData.name || userData.user_name || userData.full_name)
        ) {
          setOrgName(userData.name || userData.user_name || userData.full_name);
        } else if (userData && userData.user_id) {
          // if the endpoint returns a UserSummary shape, try common keys
          setOrgName(userData.name || null);
        } else {
          setOrgName(null);
        }
      } catch (e) {
        setOrgError("Failed to load organization name");
        setOrgName(null);
      } finally {
        setOrgLoading(false);
      }
    ...
    try {
      ...
      const mockContributors = getMockData(
        `pub/events/${event.event_id}/contributions`
      );
      setContributors(mockContributors);
      setContributorsError(
        "API connectivity issue: Unable to fetch contributor data. Showing sample contributors."
      );
    } catch (err) {
      console.error("Error handling contributors:", err);
      setContributorsError(
        "Failed to load contributors data due to connectivity issues."
      );

      // Provide mock data even if everything fails
      const fallbackContributors = getMockData(
        `pub/events/${event.event_id}/contributions`
      );
    ...
    <>
              ...
              <div className="space-y-4">
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    ...
                    <p className="mt-1">
                      {selectedEvent.start_time || "Not specified"}
                    </p>
                  </div>
                  <div>
                    <h4 className="text-sm font-medium text-gray-500">
                    ...
                    <p className="mt-1">
                      {selectedEvent.end_time || "Not specified"}
                    </p>
                  </div>
                </div>
                <div>
                  ...
                  <p className="mt-1">
                    {selectedEvent.required_volunteers || "Not specified"}
                  </p>
                </div>
                <div>
                  <h4 className="text-sm font-medium text-gray-500">
                ...
                <div>
                  ...
                  {contributorsLoading ? (
                    ...
                    <div className="text-gray-500">
                      No contributors found for this event.
                    </div>
                  ) : (
                    <ul className="mt-2 space-y-2">
                      {contributors.map((contrib) => (
                        <li
                          key={contrib.volunteer_id}
                          className="border-b pb-2"
                        >
                          <span className="font-semibold">
                            {contrib.name ||
                              `Volunteer #${contrib.volunteer_id}`}
                          </span>
                          {contrib.total_hours !== undefined && (
                            <span className="ml-2 text-sm text-gray-600">
                              Hours: {contrib.total_hours}
                            </span>
                          )}
                          {contrib.avg_rating !== undefined && (
                            <span className="ml-2 text-sm text-gray-600">
                              Avg Rating: {contrib.avg_rating}
                            </span>
                          )}
                        </li>
                      ))}
      ...
      <ConfirmModal
        ...
        onConfirm={async () => {
          try {
            const token = localStorage.getItem('token');
            const response = await fetch(`/api/org/events/${eventToDelete.event_id}`, {
              method: 'DELETE',
              headers: {
                'Authorization': `Bearer ${token}`
              }
            ...
            if (response.ok) {
              ...
              if (selectedEvent && selectedEvent.event_id === eventToDelete?.event_id) {
                setSelectedEvent(null);
              }
            } else {
