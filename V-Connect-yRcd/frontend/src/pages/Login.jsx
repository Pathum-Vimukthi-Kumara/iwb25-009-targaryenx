...
const Login = () => {
  ...
  const handleOrganizationSignup = async (e) => {
    ...
    try {
      // Register the organization using the new API structure
      const response = await fetch("/api/auth/register", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
      ...
      setError("");
      alert(
        "Registration successful! Your organization account is now pending approval. You can log in once approved."
      );
    } catch (error) {
      setError(error.message);
    } finally {
                      ...
                      {signupType === "volunteer" ? (
                        <form onSubmit={handleVolunteerSignup}>
                          ...
                          <div className="mb-4">
                            <label
                              className="block text-gray-700 text-sm font-medium mb-2"
                              htmlFor="volunteer-confirm-password"
                            >
                              Confirm Password
                            </label>
                            <div className="relative">
