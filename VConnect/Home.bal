...
function createContactMessage(ContactRequest req) returns ContactMessage|error {
    ...
    if name == "" { return error("ValidationError", message = "name required"); }
    
    string email = req.email.trim();
    if email == "" { return error("ValidationError", message = "email required"); }
    
    string subject = req.subject.trim();
    if subject == "" { return error("ValidationError", message = "subject required"); }
    
    string message = req.message.trim();
    if message == "" { return error("ValidationError", message = "message required"); }
